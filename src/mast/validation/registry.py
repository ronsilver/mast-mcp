...
def register(strategy: object) -> None:
    """Register a strategy instance. Existing entries with the same name
    take precedence (built-in loaded first).

    Accepts any object exposing a string `name` attribute and a
...
def load_from_dir(path: str | Path) -> int:
    ...
    Each `.py` file in `path` is imported; any `Strategy` subclass or
    instance defined at module level is registered. Malformed files
